...
npm run build:gh   # Build with /LocaLens/ base path for GitHub Pages
npm run lint       # ESLint (zero warnings tolerance)
npm run deploy     # Build for GH Pages + publish via gh-pages
```
...
**Deployment**: Cloudflare Pages is the primary target (`npm run build` → `dist/`). GitHub Pages is supported via `build:gh` which sets the `/LocaLens/` base path. The app is fully static — no backend.
